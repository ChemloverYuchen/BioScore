...
#     model_location = "esm2_t33_650M_UR50D"
    ...
    for seq in tqdm(seqs):
        valid_symbols = VOCAB.idx2symbol[4:24]
        valid_seq = ''.join([char if char in valid_symbols else 'X' for char in seq])
        assert len(valid_seq) == len(seq)
        
        seq_feat = seq_feature(valid_seq)
#         token_repr, contact_map_proba, logits = esm_extract(model, batch_converter, valid_seq, layer=33, approach='last', dim=1280)
        token_repr, contact_map_proba, logits = esm_extract(model, batch_converter, valid_seq, 
                                           vocab_size=len(alphabet), layer=6, approach='last', dim=320)
        
        assert len(contact_map_proba) == len(valid_seq)
...
def esm_extract(model, batch_converter, seq, vocab_size=33, layer=36, approach='mean',dim=2560):
    pro_id = 'A'
    if len(seq) <= 700:
        data = []
        ...
        for s in range(i):
            ...
            print_log(results['logits'][0].shape)
