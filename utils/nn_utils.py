...
def _knn_edges(dist, src_dst, k_neighbors, batch_info):
    ...
    dist_neighbors = dist_neighbors.masked_select(is_valid)
    ...
    assert edges.size(1) == dist_neighbors.size(0),  f"edges.size(1) :{edges.size(1)}, dist_neighbors.size(0): {dist_neighbors.size(0)} "
    return edges, dist_neighbors  # [2, E]
...
def _radial_edges_intra(dist, src_dst, segment_ids, symbol, dist_cut_offs=[10.0, 2.0, 10.0]):
    src_segments = symbol[src_dst[:, 0]]
    dst_segments = symbol[src_dst[:, 1]]
    cutoff = dist_cut_offs[src_segments]
    cutoff_dst = dist_cut_offs[dst_segments]

    assert torch.equal(cutoff, cutoff_dst), "cutoff and cutoff_dst are not the same!"

    is_valid = (dist < cutoff) & (dist < cutoff_dst)
