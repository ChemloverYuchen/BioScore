...
class AffinityTrainer(Trainer):
    ...
    def __init__(self, model, train_loader, valid_loader, config):
        ...
        print(f" current weight_decay :{self.weight_decay}")
    ...
    def get_optimizer(self):
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.config.lr, weight_decay=self.weight_decay)
        return optimizer

#     def get_optimizer(self):
#         model_params = [p for name, p in self.model.named_parameters() if name != 'alpha']
#         optimizer = torch.optim.Adam([
#             {'params': model_params, 'lr': self.config.lr},
#             {'params': [self.model.alpha], 'lr': 1e-2}
