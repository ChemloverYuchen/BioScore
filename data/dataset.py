...
# from utils.nn_utils import BatchEdgeConstructor, _knn_edges
# from models.GET.modules.tools import _block_edge_dist
...
class Block:   
    ...
    def to_data(self):
        ...
        x, a, positions, atom_features, res_features = [], [], [], [], []
        block_len = len(self)
...
def blocks_to_data(*blocks_list: List[List[Block]]):
    ...
    for i, blocks in enumerate(blocks_list):
        ...
        cur_segment_ids = [i for _ in cur_B] # [i,i,i,i...]
...
class BlockGeoAffDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file, database=None, dist_th=6, n_cpu=48, suffix=''):
        '''
...
#             data = [self._preprocess(item) for i, item in enumerate(tqdm(items))]
            ...
            if isinstance(items, list):
                data = process_map(self._preprocess, items, max_workers=n_cpu, chunksize=5)
            else:  # LMDB
                print('Data not list, disable parallel processing')
                from tqdm import tqdm
                data = [self._preprocess(item) for i, item in enumerate(tqdm(items))]
                
            data = list(filter(self.condition, data))
    ...
    def _post_process(self, items, processed_data):
        if 'rmsd' in items[0]:
            indexes = [ { 'id': item['id'], 'affinity': item['affinity'], 'rmsd': item['rmsd'] } for item, d in zip(items, processed_data) if d is not None ]
        elif 'dockq' in items[0]:
            indexes = [ { 'id': item['id'], 'affinity': item['affinity'], 'dockq': item['dockq'] } for item, d in zip(items, processed_data) if d is not None ]
        elif 'native' in items[0]:
            indexes = [ { 'id': item['id'], 'affinity': item['affinity'], 'native': item['native'] } for item, d in zip(items, processed_data) if d is not None ]
        else:
    ...
    def __getitem__(self, idx):
        ...
        {
            'X': [Natom, n_channel, 3],   np
            'B': [Nblock],   list
            'A': [Natom],    list
            'atom_positions': [Natom],
            'block_lengths': [Natom]
            'segment_ids': [Nblock]   00000001111111
            'label': [1] 
        }
        '''
        item = self.data[idx]
        item['symbol'] = [0 for _ in item['segment_ids']] #  block level, means whether the block is protein 0 or molecule 1  or nuclear acid 2
        
        return item

    @classmethod
    def collate_fn(cls, batch):
        keys = ['X', 'B', 'A', 'atom_positions', 'block_lengths', 'segment_ids', 'symbol']
...
#For NLI
class NLIDataset(BlockGeoAffDataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        item = self.data[idx]
        item['symbol'] = [1 if x == 1 else 0 for x in item['segment_ids']] 
        assert len(item['symbol']) == len(item['segment_ids']) #  block level,  means all blocks from this dataset is protein
...
class PDBBindBenchmark(torch.utils.data.Dataset):
    ...
    def __init__(self, data_file):
        ...
        # to satify the requirements of inference.py
        if 'rmsd' in self.data[0]:
            self.indexes = [ {'id': item['id'], 'label': item['affinity']['neglog_aff'], 'rmsd': item['rmsd'] } for item in self.data ]
        elif 'native' in self.data[0]:
            self.indexes = [ {'id': item['id'], 'label': item['affinity']['neglog_aff'], 'native': item['native'] } for item in self.data ]    
        else:
            self.indexes = [ {'id': item['id'], 'label': item['affinity']['neglog_aff'] } for item in self.data ]
    def __len__(self):
        return len(self.data)
...
#         print(f" pdb id: {item['id']}")
        ...
        data['symbol'] = [1 if x == 1 else 0 for x in data['segment_ids']] 
        assert len(data['symbol']) == len(data['segment_ids']) #  block level,  means all blocks from this dataset is protein
