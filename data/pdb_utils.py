...
class Protein:
    ...
    def from_pdb(cls, pdb_path, include_all=False):
        ...
        if pdb_id == '':
            ...
            pdb_id = os.path.splitext(os.path.basename(pdb_path))[0] + '(filename)'
...
class Complex(Protein):
    ...
    def get_interacting_residues(self, dist_th=10):
        ...
        rec_seqs, lig_seqs = [], []
        for res_list, chains, seqs in zip([rec_residues, lig_residues], [self.receptor_chains, self.ligand_chains], [rec_seqs, lig_seqs]):
            for chain in chains:
                for residue in self.peptides[chain]:
                    res_list.append((chain, residue))
     
                    seqs.append(residue.get_symbol())
        ...
        print(f'dist min: {np.min(dist)}, max: {np.max(dist)}')
        rec_inter = [rec_residues[i] for i in rec_index]
        lig_inter = [lig_residues[i] for i in lig_index]
        print(f"Length of rec_residues: {len(rec_residues)}")
        print(f"Length of rec_seqs: {len(rec_seqs)}")
        
        if len(rec_inter) == 0 or len(lig_inter) == 0:
            max_rec_index = - np.inf
            max_lig_index = - np.inf
        else:
            max_rec_index = max(rec_index)
            max_lig_index = max(lig_index)
            
        print(f"Max rec_index: {max_rec_index}")
        assert len(rec_residues)==len(rec_seqs) and max_rec_index < len(rec_seqs)
        assert len(lig_residues)==len(lig_seqs) and max_lig_index < len(lig_seqs)
        return rec_inter, lig_inter, rec_index, lig_index, "".join(rec_seqs), "".join(lig_seqs)
    
    def get_interacting_residues_analysis(self, dist_th=10):
        '''
        calculate interacting residues based on minimum distance between heavy atoms < 10A (default)
        '''
        rec_residues, lig_residues = [], []
        ...
        dist_min_max = (np.min(dist), np.max(dist))
        print(f'dist min: {dist_min_max[0]}, max: {dist_min_max[1]}')
        ...
        assert len(rec_residues)==len(rec_seqs) and max_rec_index < len(rec_seqs)
        assert len(lig_residues)==len(lig_seqs) and max_lig_index < len(lig_seqs)
        return rec_inter, lig_inter, rec_index, lig_index, "".join(rec_seqs), "".join(lig_seqs), dist_min_max
