...
def parse():
    ...
    parser.add_argument('--split_file', type=str, required=True,
                        help='Path to the split info file')
    parser.add_argument('--pdb_dir', type=str, required=True,
                        help='Directory of pdbs: PP')
    parser.add_argument('--out_dir', type=str, required=True,
                        help='Output directory')
    parser.add_argument('--outname', type=str, default='PDBbind_PN',
...
def process_line(line, pdb_dir, interface_dist_th):
    ...
    # construct pockets
    interface1, interface2, _, _, _, _ = cplx.get_interacting_residues(dist_th=interface_dist_th)
...
def main(args):
    ...
    # save to file（all, RNA, DNA）
    database_out = os.path.join(args.out_dir, f'{args.outname}_all.pkl')
    ...
    # split by RNA/DNA (according to PRA310.csv)
    with open(args.split_file, 'r') as f:
        split_dict = json.load(f)    
    
    rna_ids = split_dict['RNA']
    print_log(f'RNA in PDBbind NL: {len(rna_ids)}')
    
    rna_items, dna_items = [], []
    for item in processed_pdbbind:
        if item['id'] in rna_ids:
            rna_items.append(item)
        else:
            dna_items.append(item)
    
    # RNA
    database_rna = os.path.join(args.out_dir, f'{args.outname}_RNA.pkl')
    print_log(f'Obtained {len(rna_items)} data after filtering, saving to {database_rna}...')
    with open(database_rna, 'wb') as fout:
        pickle.dump(rna_items, fout) 
    
    # DNA
    database_dna = os.path.join(args.out_dir, f'{args.outname}_DNA.pkl')
    print_log(f'Obtained {len(dna_items)} data after filtering, saving to {database_dna}...')
    with open(database_dna, 'wb') as fout:
        pickle.dump(dna_items, fout)
