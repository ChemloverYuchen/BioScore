...
def parse():
    ...
    parser.add_argument('--pdb_dir', type=str, required=True,
                        help='Directory of all pdb dirs, each pdb dir: 1 native + 100 decoys + info')
...
def process_line(line, root, pdb_id, interface_dist_th):
    line = line.split(',')
    assert len(line) == 3
    struct_idx, (rec_chains, lig_chains) = line[0], line[2].strip().split("_")  # e.g. decoy_0,0.0107,E_I
    dockq = float(line[1])
    
    lig_chains, rec_chains = list(lig_chains), list(rec_chains)
    item = {
            'id': f'{pdb_id}_{struct_idx}',
            'dockq': dockq
    }
    item['affinity'] = {
            'Kd': 0.0,
    ...
    cplx_path = os.path.join(root, pdb_id, f'{struct_idx}.pdb')
