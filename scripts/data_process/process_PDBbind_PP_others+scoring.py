...
def parse():
    ...
    parser.add_argument('--outname', type=str, default='PDBbind_PP',
...
def process_line(line, pdb_dir, interface_dist_th):
    ...
    fasta_file = os.path.join(pdb_dir, 'fasta', f'{pdb}.txt')
    with open(fasta_file, 'r') as f:
        fasta = f.read()
    fasta = fasta.strip().split('\n')
    temp_proteins = parse_fasta(fasta)
    if len(temp_proteins) != 2:  # irregular fasta, cannot distinguish which chains composes one protein
        print_log(f'{pdb} has {len(temp_proteins)} chain sets!', level='ERROR')
        return None
    
    # rank by seq lengths ([longer, shorter])
    assert len(temp_proteins) == 2
    proteins = sorted(temp_proteins, key=lambda x: len(x['seq']), reverse=True)
...
def main(args):
    ...
    # split train/valid and scoring benchmark
    print_log('Split train/valid and scoring benchmark ...')
    database_benchmark = os.path.join(args.out_dir, f'{args.outname}_scoring.pkl')
    database_others = os.path.join(args.out_dir, f'{args.outname}_others.pkl')

    # PPIBenchmark（79 pdb_ids）
    benchmark_ids = [
        '6f0f', '1j2j', '1mzw', '5yip', '4js0', '3h8k', '2vay', '5wuj', '6j4s', '6e3j',
        '2wh6', '5gtb', '6jjw', '5h3j', '5inb', '2gww', '5h9b', '3ukz', '5ky4', '2qxv',
        '6bw9', '4uyq', '2hth', '3ul4', '1gua', '4dbg', '3u43', '3fpu', '1kac', '6ne4',
        '2v9t', '5ml9', '2uyz', '4eig', '1lw6', '3vv2', '3zu7', '5jw7', '5g15', '2f4m',
        '4z9k', '4d0n', '6hul', '1tdq', '2qc1', '2v8s', '2omz', '2voh', '3gj6', '3kj0',
        '3tz1', '4d0g', '4rey', '4y61', '1pjm', '1pjn', '1syq', '1rkc', '1t01', '3ukx',
        '3ul0', '3ul1', '5ky5', '6iu7', '6iua', '6k06', '1emv', '1p69', '2vlp', '2vln',
        '3kuc', '2sic', '1jiw', '2b7c', '2omw', '2v3b', '2z58', '3vyr', '4m0w'
    ]
    ...
    for item in processed_pdbbind:
        if item['id'] in benchmark_ids:
            benchmark.append(item)
        else:
            others.append(item)

    print_log(f'Obtained {len(processed_pdbbind)} data after filtering ...')
    print_log(f'Benchmark: {len(benchmark)} data, saving to {database_benchmark}...')
    with open(database_benchmark, 'wb') as fout:
        pickle.dump(benchmark, fout)
    
    print_log(f'Others: {len(others)} data, saving to {database_others}...')
    with open(database_others, 'wb') as fout:
        pickle.dump(others, fout)
